...
int ADC_SECO = 22000;    // <-- cambia con tu medición real
int ADC_HUMEDO = 11000;  // <-- cambia con tu medición real
...
void actualizarUI(int humedad) {
    ...
    while ((child = lv_obj_get_child(scr, i)) != NULL) {
        ...
        if (lv_obj_check_type(child, &lv_label_class)) {
            ...
            if (txt &&
                ...
                strcmp(txt, "ANALIZADOR DE HIDRATACION DE LA PIEL") != 0) {
    ...
    if (arc_obj) {
        lv_arc_set_value(arc_obj, humedad);
    }
...
void setup() {
  ...
  digitalWrite(PIN_RST_TOUCH, LOW); delay(50);
  digitalWrite(PIN_RST_TOUCH, HIGH); delay(200);
  ...
  if (ads.begin(0x48)) {
    ...
    USBSerial.println("ADS1115 detectado");
  } else {
    USBSerial.println("ADS1115 NO detectado");
  }
...
void loop() {
  ...
  if (adsConectado) {
    ...
    if (millis() - lastS > 1000) {
      ...
      // 🔥 CONVERSIÓN CALIBRADA
      hidratacionActual = map(adc0, ADC_SECO, ADC_HUMEDO, 0, 100);
      hidratacionActual = constrain(hidratacionActual, 0, 100);

      actualizarUI(hidratacionActual);

      // 🔬 MONITOR PARA CALIBRAR
      USBSerial.printf("ADC: %d | Humedad: %d%%\n", adc0, hidratacionActual);
    }
  }
